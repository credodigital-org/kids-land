...
function NewsletterPage() {
  ...
  async function load() {
    ...
    setError("");
    try {
      setVersions(await newsletterService.listNewsletterVersions());
    } catch (err) {
      setError("Could not load newsletter versions.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleUpload(e) {
    ...
    setUploading(true);
    try {
      await newsletterService.uploadNewsletter({ title: "Newsletter", pdfFile: file });
      setFile(null);
      await load();
    } catch (err) {
  ...
  async function handleSaveEdit(e) {
    ...
    setSavingEdit(true);
    try {
      await newsletterService.updateNewsletterVersion(editingItem.id, { title: editTitle });
      cancelEdit();
      await load();
    } catch (err) {
  ...
  async function confirmDelete() {
    await newsletterService.deleteNewsletterVersion(toDelete.id);
    ...
    <AdminLayout title="Newsletter">
      ...
      {editingItem ? (
        ...
        <div className="content-form-card">
          ...
          <form className="content-upload-form" onSubmit={handleUpload}>
            ...
            <div className="content-form-field content-form-actions">
              <button type="submit" disabled={uploading}>{uploading ? "Uploading..." : "Upload New Newsletter"}</button>
            </div>
          </form>
        </div>
      )}
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No newsletter uploaded yet."
        />
      )}

      <ConfirmDialog
        open={!!toDelete}
        message="Delete this newsletter version?"
